...
class Evaluation(object):
    ...
    def run_episodes(self):
        for episode in range(self.num_episodes):
            ...
            while not terminal:
                ...
                #option 1:
                reward_1,reward_2,reward_3,terminal = self.step()
                #option 2:
                #reward_all, terminal = self.step()
            ...
            for i in range(len(rewards_1)):
                rewards_all.append(rewards_1[i] + rewards_2[i] + rewards_3[i])
            self.after_all_episodes(episode, rewards_all )
            #self.after_all_episodes(episode, rewards_1,rewards_2,rewards_3)
            #self.after_some_episodes(episode, rewards)
            #self.after_some_episodes(episode, rewards_1,rewards_2,rewards_3)
            self.after_some_episodes(episode, rewards_all)
    ...
    def step(self):
       ...
       # self.observation, reward, terminal, info = self.monitor.step(action)
        #option 1:
        self.observation, reward_1, reward_2, reward_3, terminal, info = self.monitor.step(action)
        reward_all = reward_1+reward_2+reward_3
        #option 2:
        #self.observation, reward_all, terminal, info = self.monitor.step(action)
        ...
        try:
            ...
            #option 1:
            #self.agent.record(previous_observation, action, reward_1, reward_2, reward_3, self.observation, terminal, info)
            #option 3:
            self.agent.record(previous_observation, action, reward_all,  reward_1, reward_2, reward_3, self.observation, terminal,
                              info)
        ...
        #option 1:
        return reward_1, reward_2, reward_3, terminal
        #option 2:
        #return reward_all, terminal
    ...
    def after_all_episodes(self, episode, rewards_all):
        ...
        self.writer.add_scalar('episode/length', len(rewards_all), episode)
        #self.writer.add_scalar('episode/total_reward', sum(rewards), episode)
        self.writer.add_scalar('episode/total_reward', sum(rewards_all), episode)
        #self.writer.add_scalar('episode/return', sum(r*gamma**t for t, r in enumerate(rewards)), episode)
        self.writer.add_scalar('episode/return', sum(r * gamma ** t for t, r in enumerate(rewards_all)), episode)
        #self.writer.add_histogram('episode/rewards', rewards, episode)
        self.writer.add_histogram('episode/rewards', rewards_all, episode)
        #logger.info("Episode {} score: {:.1f}".format(episode, sum(rewards)))
        logger.info("Episode {} score: {:.1f}".format(episode, sum(rewards_all)))

    #yaeli
    #def after_all_episodes(self, episode, rewards_1,rewards_2,rewards_3):
     #   rewards_1 = np.array(rewards_1)
      #  rewards_2 = np.array(rewards_2)
       # rewards_3 = np.array(rewards_3)
        #all_rewards = rewards_1+rewards_2+rewards_3
        #gamma = self.agent.config.get("gamma", 1)
        #self.writer.add_scalar('episode/length', len(rewards_1), episode)
        #self.writer.add_scalar('episode/length', len(all_rewards), episode)
        #self.writer.add_scalar('episode/total_reward', sum(rewards_1), episode)
        #self.writer.add_scalar('episode/total_reward', sum(all_rewards), episode)
        #self.writer.add_scalar('episode/return', sum(r*gamma**t for t, r in enumerate(rewards)), episode)
        #self.writer.add_scalar('episode/return', sum(r * gamma ** t for t, r in enumerate(all_rewards)), episode)
        #self.writer.add_histogram('episode/rewards', rewards, episode)
        #self.writer.add_histogram('episode/rewards', all_rewards, episode)
        #logger.info("Episode {} score: {:.1f}".format(episode, sum(rewards)))
        #logger.info("Episode {} score: {:.1f}".format(episode, sum(all_rewards)))

#yaeli
    def after_some_episodes(self, episode, rewards,
                            best_increase=1.1,
                            episodes_window=50):
    #def after_some_episodes(self, episode, rewards_1,rewards_2,rewards_3,
          #                  best_increase=1.1,
         #                   episodes_window=50):
        #all_rewards = rewards_1 + rewards_2 + rewards_3
        ...
        if self.training:
            ...
            #self.filtered_agent_stats += 1 / episodes_window * (np.sum(all_rewards) - self.filtered_agent_stats)
