...
class MultiLayerPerceptron(BaseModule, Configurable):
    def __init__(self, config):
        ...
        if self.config.get("out", None):
            ...
            self.predict_1 = nn.Linear(sizes[-1], self.config["out"])
            self.predict_2 = nn.Linear(sizes[-1], self.config["out"])
            self.predict_3 = nn.Linear(sizes[-1], self.config["out"])
    ...
    def forward(self, x):
        ...
        if self.config.get("out", None):
            ...
            # option 2:
            #y = torch.cat((x1,x2))
           # x= torch.cat((x_sum,x1,x2,x3),dim=2)
            #s = torch.squeeze(y, 1)
            #s = torch.stack((x1, x2), dim=-1)
        all_x = x_sum,x1,x2,x3
        all_x = torch.stack(list(all_x), dim=0)
        return all_x
        #return x_sum,x1,x2,x3
        #return x
