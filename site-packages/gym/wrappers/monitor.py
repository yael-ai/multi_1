...
class Monitor(Wrapper):
    ...
    def step(self, action):
        ...
        #option 1:
        observation, reward_1,reward_2,reward_3, done, info = self.env.step(action)
        #option 2:
        #observation, reward_all, done, info = self.env.step(action)


        #original:
        #done = self._after_step(observation, reward, done, info)
        #option 1:
        done = self._after_step(observation, reward_1,reward_2,reward_3, done, info)
        #option 2:
        #done = self._after_step(observation, reward_all, done, info)
        #reward_all = reward_1 + reward_2 + reward_3

        #original:
        #return observation, reward, done, info
        #option 1:
        return observation, reward_1,reward_2,reward_3, done, info
        #option 2:
        #return observation, reward_all, done, info
 ...
 You should use a unique directory for each training run, or use 'force=True' to automatically clear previous monitor files.'''.format(directory, ', '.join(training_manifests[:5])))
    ...
    #def _after_step(self, observation, reward_all, done, info):
     #   if not self.enabled: return done
      ...
      #  if done and self.env_semantics_autoreset:
       ...
       #     self.reset_video_recorder()
        ...
        #self.stats_recorder.after_step(observation, reward_all, done, info)
        # Record video
    ...
    #option 1:
    def _after_step(self, observation, reward_1,reward_2,reward_3, done, info):
        if not self.enabled: return done
        ...
        # Record stats
        self.stats_recorder.after_step(observation, reward_1,reward_2,reward_3, done, info)
        # Record video
        self.video_recorder.capture_frame()
        return done
